...
The CLARISSA Portal is being built during an experimental phase by ~4 developers, but should later be used productively by significantly more users. The observability strategy must:

1. **Now**: Minimal, free, quick to deploy
2. **Later**: Scalable for production workloads
3. **Always**: Set up correctly from the start (Structured Logging, Correlation IDs)

---

## Decision

### Strategy: "Design for Scale, Start Simple"

| Phase | Users | Stack | Cost |
|-------|-------|-------|------|
| **Phase 1** (now) | 4 Devs | GCP Native | $0 |
| **Phase 2** (50+ Users) | Early Adopters | + Sentry | ~$0-26/mo |
| **Phase 3** (500+ Users) | Production | Full Stack | $50-500/mo |
...
| **Alerting** | GCP Alerts → Email/Slack | Minimal but effective |
| **Tracing** | OpenTelemetry (prepared) | Future-proof, not yet active |

---

## Phase 1: Experimental Phase (Now)
...
│   │                     Error Reporting                                  │   │
│   │                                                                      │   │
│   │  • Stack Traces (automatic from logs)                               │   │
...
- ❌ Custom Dashboards (GCP Console is sufficient)
- ❌ Distributed Tracing (only 2 services)
...
|-------|------|---------|--------|
| **ERROR** | Needs attention | PDF Generation failed | ✅ Yes |
| **WARNING** | Unusual but OK | Retry succeeded, Rate limit near | No |
| **INFO** | Normal operations | Request completed, User logged in | No |
...
### Who Generates the Correlation ID?
  ...
  - name: "CLARISSA - Service Unavailable"
    condition:
      filter: |
        ...
        resource.labels.service_name="clarissa-portal-api"
        metric.type="run.googleapis.com/request_count"
      absence_duration: 600s  # 10 minutes no traffic
    notification:
      - email: wolfram.laube@blauweiss-edv.at
...
async function apiCall(endpoint, options = {}) {
    ...
    if (!response.ok) {
        // User can send this to us for support
...
## Phase 2: Early Adopters (50+ Users)
...
**Cost:** $0 (Developer) / $26/mo (Team)

---

## Phase 3: Production (500+ Users)

### Option A: GCP Native (simpler)
...
### Option B: Grafana Stack (more control)
...
**Decision will be made when we get there** - the important thing is that we're prepared through OpenTelemetry.

---

## OpenTelemetry Preparation

Even though we don't actively use tracing in Phase 1, we instrument from the start with OpenTelemetry. This makes a later switch to any backend trivial.
...
|------|---------|---------|---------|
| **Logging** | structlog → Cloud Logging | same | + Log Analytics |
| **Metrics** | Cloud Run built-in | same | + Custom Dashboards |
| **Errors** | Cloud Error Reporting | + Sentry | Sentry |
| **Tracing** | (prepared) | (prepared) | Cloud Trace / Tempo |
| **Alerting** | Email + Slack | same | PagerDuty/Opsgenie |
| **Cost** | $0 | ~$26/mo | $50-500/mo |

### Non-Negotiable (all phases)

1. ✅ **Structured Logging** (JSON, not print())
2. ✅ **Correlation IDs** (Portal API generates, propagates)
...
- [ ] Worker: Read Correlation ID from header
- [ ] Log-based Metric: auth_failures
- [ ] Alert: Error Rate > 10%
- [ ] Alert: Service Down
- [ ] Frontend: Display Correlation ID in errors
...
- [ ] Activate OpenTelemetry Exporter
- [ ] Custom Dashboards
